...
#include "EsotericUserSubsystem.generated.h"

class FNativeGameplayTag;
class IOnlineSubsystem;

/** List of tags used by the esoteric user subsystem */
struct ESOTERICUSER_API FEsotericUserTags
{
...
class ESOTERICUSER_API UEsotericUserInfo : public UObject
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(BlueprintReadOnly, Category = UserInfo)
	EEsotericUserInitializationState InitializationState = EEsotericUserInitializationState::Invalid;
	...
	UFUNCTION(BlueprintCallable, Category = UserInfo)
	EEsotericUserPrivilegeResult GetCachedPrivilegeResult(EEsotericUserPrivilege Privilege, EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;

	/** Ask about the general availability of a feature, this combines cached results with state */
	UFUNCTION(BlueprintCallable, Category = UserInfo)
	EEsotericUserAvailability GetPrivilegeAvailability(EEsotericUserPrivilege Privilege) const;

	/** Returns the net id for the given context */
	UFUNCTION(BlueprintCallable, Category = UserInfo)
	FUniqueNetIdRepl GetNetId(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;
	...
	struct FCachedData
	{
		...
		TMap<EEsotericUserPrivilege, EEsotericUserPrivilegeResult> CachedPrivileges;
	};

	/** Per context cache, game will always exist but others may not */
	TMap<EEsotericUserOnlineContext, FCachedData> CachedDataMap;
	
	/** Looks up cached data using resolution rules */
	FCachedData* GetCachedData(EEsotericUserOnlineContext Context);
	const FCachedData* GetCachedData(EEsotericUserOnlineContext Context) const;

	/** Updates cached privilege results, will propagate to game if needed */
	void UpdateCachedPrivilegeResult(EEsotericUserPrivilege Privilege, EEsotericUserPrivilegeResult Result, EEsotericUserOnlineContext Context);

	/** Updates cached privilege results, will propagate to game if needed */
	void UpdateCachedNetId(const FUniqueNetIdRepl& NewId, EEsotericUserOnlineContext Context);

	/** Return the subsystem this is owned by */
	class UEsotericUserSubsystem* GetSubsystem() const;
};


/** Delegates when initialization processes succeed or fail */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FiveParams(FEsotericUserOnInitializeCompleteMulticast, const UEsotericUserInfo*, UserInfo, bool, bSuccess, FText, Error, EEsotericUserPrivilege, RequestedPrivilege, EEsotericUserOnlineContext, OnlineContext);
DECLARE_DYNAMIC_DELEGATE_FiveParams(FEsotericUserOnInitializeComplete, const UEsotericUserInfo*, UserInfo, bool, bSuccess, FText, Error, EEsotericUserPrivilege, RequestedPrivilege, EEsotericUserOnlineContext, OnlineContext);

/** Delegate when a system error message is sent, the game can choose to display it to the user using the type tag */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FEsotericUserHandleSystemMessageDelegate, FGameplayTag, MessageType, FText, TitleText, FText, BodyText);

/** Delegate when a privilege changes, this can be bound to see if online status/etc changes during gameplay */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FEsotericUserAvailabilityChangedDelegate, const UEsotericUserInfo*, UserInfo, EEsotericUserPrivilege, Privilege, EEsotericUserAvailability, OldAvailability, EEsotericUserAvailability, NewAvailability);

/** Parameter struct for initialize functions, this would normally be filled in by wrapper functions like async nodes */
USTRUCT(BlueprintType)
struct ESOTERICUSER_API FEsotericUserInitializeParams
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Default)
	EEsotericUserPrivilege RequestedPrivilege = EEsotericUserPrivilege::CanPlay;

	/** What specific online context to log in to, game means to login to all relevant ones */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Default)
	EEsotericUserOnlineContext OnlineContext = EEsotericUserOnlineContext::Game;
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Default)
	FEsotericUserOnInitializeComplete OnUserInitializeComplete;
};
...
class ESOTERICUSER_API UEsotericUserSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(BlueprintAssignable, Category = EsotericUser)
	FEsotericUserOnInitializeCompleteMulticast OnUserInitializeComplete;

	/** BP delegate called when the system sends an error/warning message */
	UPROPERTY(BlueprintAssignable, Category = EsotericUser)
	FEsotericUserHandleSystemMessageDelegate OnHandleSystemMessage;

	/** BP delegate called when privilege availability changes for a user  */
	UPROPERTY(BlueprintAssignable, Category = EsotericUser)
	FEsotericUserAvailabilityChangedDelegate OnUserPrivilegeChanged;

	/** Send a system message via OnHandleSystemMessage */
	UFUNCTION(BlueprintCallable, Category = EsotericUser)
	...
	UFUNCTION(BlueprintPure, Category = EsotericUser)
	...
	UFUNCTION(BlueprintPure, Category = EsotericUser)
	EEsotericUserInitializationState GetLocalPlayerInitializationState(int32 LocalPlayerIndex) const;

	/** Returns the user info for a given local player index in game instance, 0 is always valid in a running game */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	const UEsotericUserInfo* GetUserInfoForLocalPlayerIndex(int32 LocalPlayerIndex) const;

	/** Deprecated, use PlatformUserId when available */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	const UEsotericUserInfo* GetUserInfoForPlatformUserIndex(int32 PlatformUserIndex) const;

	/** Returns the primary user info for a given platform user index. Can return null */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	const UEsotericUserInfo* GetUserInfoForPlatformUser(FPlatformUserId PlatformUser) const;

	/** Returns the user info for a unique net id. Can return null */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	const UEsotericUserInfo* GetUserInfoForUniqueNetId(const FUniqueNetIdRepl& NetId) const;

	/** Deprecated, use InputDeviceId when available */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	const UEsotericUserInfo* GetUserInfoForControllerId(int32 ControllerId) const;

	/** Returns the user info for a given input device. Can return null */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	const UEsotericUserInfo* GetUserInfoForInputDevice(FInputDeviceId InputDevice) const;
	 ...
	 * When the process has succeeded or failed, it will broadcast the OnUserInitializeComplete delegate.
	 * AsyncAction_EsotericUserInitialize provides several wrapper functions for using this in an Event graph.
	 *
	 * @returns true if the process was started, false if it failed before properly starting
	 */
	UFUNCTION(BlueprintCallable, Category = EsotericUser)
	virtual bool TryToInitializeUser(FEsotericUserInitializeParams Params);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = EsotericUser)
	virtual void ListenForLoginKeyInput(TArray<FKey> AnyUserKeys, TArray<FKey> NewUserKeys, FEsotericUserInitializeParams Params);
	...
	/** Returns true if this could be a real platform user with a valid identity (even if not currently logged in)  */
	...
	UFUNCTION(BlueprintPure, BlueprintPure, Category=EsotericUser)
	...
	IOnlineSubsystem* GetOnlineSubsystem(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;

	/** Returns identity interface of specific type, will return null if there is no type */
	IOnlineIdentity* GetOnlineIdentity(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;

	/** Returns human readable name of OSS system */
	UFUNCTION(BlueprintCallable, BlueprintPure = False, Category = EsotericUser)
	FName GetOnlineSubsystemName(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;

	/** Returns the current online connection status */
	EOnlineServerConnectionStatus::Type GetConnectionStatus(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;


	/** Returns true if we are currently connected to backend servers */
	bool HasOnlineConnection(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;

	/** Returns the current login status for a player on the specified online system, only works for real platform users */
	ELoginStatusType GetLocalUserLoginStatus(FPlatformUserId PlatformUser, EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;

	/** Returns the unique net id for a local platform user */
	FUniqueNetIdRepl GetLocalUserNetId(FPlatformUserId PlatformUser, EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;
	...
	FString EEsotericUserOnlineContextToString(EEsotericUserOnlineContext Context);

	/** Returns human readable string for privilege checks */
	virtual FText GetPrivilegeDescription(EEsotericUserPrivilege Privilege) const;
	virtual FText GetPrivilegeResultDescription(EEsotericUserPrivilegeResult Result) const;
	 ...
	 */
	DECLARE_DELEGATE_FiveParams(FOnLocalUserLoginCompleteDelegate, const UEsotericUserInfo* /*UserInfo*/, ELoginStatusType /*NewStatus*/, FUniqueNetIdRepl /*NetId*/, const TOptional<FOnlineErrorType>& /*Error*/, EEsotericUserOnlineContext /*Type*/);
	virtual bool LoginLocalUser(const UEsotericUserInfo* UserInfo, EEsotericUserPrivilege RequestedPrivilege, EEsotericUserOnlineContext Context, FOnLocalUserLoginCompleteDelegate OnComplete);

	/** Assign a local player to a specific local user and call callbacks as needed */
	virtual void SetLocalPlayerUserInfo(ULocalPlayer* LocalPlayer, const UEsotericUserInfo* UserInfo);

	/** Resolves a context that has default behavior into a specific context */
	EEsotericUserOnlineContext ResolveOnlineContext(EEsotericUserOnlineContext Context) const;
...
protected:
	...
	{
		FUserLoginRequest(UEsotericUserInfo* InUserInfo, EEsotericUserPrivilege InPrivilege, EEsotericUserOnlineContext InContext, FOnLocalUserLoginCompleteDelegate&& InDelegate)
			: UserInfo(TWeakObjectPtr<UEsotericUserInfo>(InUserInfo))
		...
		TWeakObjectPtr<UEsotericUserInfo> UserInfo;

		/** Overall state of login request, could come from many sources */
		EEsotericUserAsyncTaskState OverallLoginState = EEsotericUserAsyncTaskState::NotStarted;
		...
		EEsotericUserAsyncTaskState TransferPlatformAuthState = EEsotericUserAsyncTaskState::NotStarted;

		/** State of attempt to use AutoLogin */
		EEsotericUserAsyncTaskState AutoLoginState = EEsotericUserAsyncTaskState::NotStarted;

		/** State of attempt to use external login UI */
		EEsotericUserAsyncTaskState LoginUIState = EEsotericUserAsyncTaskState::NotStarted;

		/** Final privilege to that is requested */
		EEsotericUserPrivilege DesiredPrivilege = EEsotericUserPrivilege::Invalid_Count;

		/** State of attempt to request the relevant privilege */
		EEsotericUserAsyncTaskState PrivilegeCheckState = EEsotericUserAsyncTaskState::NotStarted;

		/** The final context to log into */
		EEsotericUserOnlineContext DesiredContext = EEsotericUserOnlineContext::Invalid;

		/** What online system we are currently logging into */
		EEsotericUserOnlineContext CurrentContext = EEsotericUserOnlineContext::Invalid;
	...
	virtual UEsotericUserInfo* CreateLocalUserInfo(int32 LocalPlayerIndex);

	/** Deconst wrapper for const getters */
	FORCEINLINE UEsotericUserInfo* ModifyInfo(const UEsotericUserInfo* Info) { return const_cast<UEsotericUserInfo*>(Info); }

	/** Refresh user info from OSS */
	virtual void RefreshLocalUserInfo(UEsotericUserInfo* UserInfo);

	/** Possibly send privilege availability notification, compares current value to cached old value */
	virtual void HandleChangedAvailability(UEsotericUserInfo* UserInfo, EEsotericUserPrivilege Privilege, EEsotericUserAvailability OldAvailability);

	/** Updates the cached privilege on a user and notifies delegate */
	virtual void UpdateUserPrivilegeResult(UEsotericUserInfo* UserInfo, EEsotericUserPrivilege Privilege, EEsotericUserPrivilegeResult Result, EEsotericUserOnlineContext Context);

	/** Gets internal data for a type of online system, can return null for service */
	const FOnlineContextCache* GetContextCache(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game) const;
	FOnlineContextCache* GetContextCache(EEsotericUserOnlineContext Context = EEsotericUserOnlineContext::Game);
	...
	virtual EEsotericUserPrivilege ConvertOSSPrivilege(EUserPrivileges::Type Privilege) const;
	virtual EUserPrivileges::Type ConvertOSSPrivilege(EEsotericUserPrivilege Privilege) const;
	virtual EEsotericUserPrivilegeResult ConvertOSSPrivilegeResult(EUserPrivileges::Type Privilege, uint32 Results) const;

	
	/** Callbacks for OSS functions */
	virtual void HandleIdentityLoginStatusChanged(int32 PlatformUserIndex, ELoginStatus::Type OldStatus, ELoginStatus::Type NewStatus, const FUniqueNetId& NewId, EEsotericUserOnlineContext Context);
	virtual void HandleUserLoginCompleted(int32 PlatformUserIndex, bool bWasSuccessful, const FUniqueNetId& NetId, const FString& Error, EEsotericUserOnlineContext Context);
	...
	virtual void HandleNetworkConnectionStatusChanged(const FString& ServiceName, EOnlineServerConnectionStatus::Type LastConnectionStatus, EOnlineServerConnectionStatus::Type ConnectionStatus, EEsotericUserOnlineContext Context);
	virtual void HandleOnLoginUIClosed(TSharedPtr<const FUniqueNetId> LoggedInNetId, const int PlatformUserIndex, const FOnlineError& Error, EEsotericUserOnlineContext Context);
	virtual void HandleCheckPrivilegesComplete(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults, EEsotericUserPrivilege RequestedPrivilege, TWeakObjectPtr<UEsotericUserInfo> EsotericUserInfo, EEsotericUserOnlineContext Context);
	...
	virtual void HandleLoginForUserInitialize(const UEsotericUserInfo* UserInfo, ELoginStatusType NewStatus, FUniqueNetIdRepl NetId, const TOptional<FOnlineErrorType>& Error, EEsotericUserOnlineContext Context, FEsotericUserInitializeParams Params);
	virtual void HandleUserInitializeFailed(FEsotericUserInitializeParams Params, FText Error);
	virtual void HandleUserInitializeSucceeded(FEsotericUserInitializeParams Params);
	...
	FEsotericUserInitializeParams ParamsForLoginKey;
	...
	UPROPERTY()
	TMap<int32, TObjectPtr<UEsotericUserInfo>> LocalUserInfos;
