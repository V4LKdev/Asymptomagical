...
#include "EsotericSessionSubsystem.generated.h"

class UWorld;
class FEsotericSession_OnlineSessionSettings;
class FEsotericOnlineSearchSettings;

const FName LOBBY_NAME_KEY{ "LobbyName" };

#pragma region Host

/** Specifies the online features and connectivity that should be used for a game session */
UENUM(BlueprintType)
enum class EEsotericSessionOnlineMode : uint8
{
	Offline,
	LAN,
	Online
...
class ESOTERICUSER_API UEsotericSession_HostSessionRequest : public UObject
{
	GENERATED_BODY()
public:
	...
	EEsotericSessionOnlineMode OnlineMode = EEsotericSessionOnlineMode::Offline;
	...
	UPROPERTY(BlueprintReadWrite, Category=Session)
	FString ModeNameForAdvertisement = "DefaultGameMode";

	/** String used during matchmaking to specify the name of the lobby */
	UPROPERTY(BlueprintReadWrite, Category=Session)
	FString LobbyNameForAdvertisement = "DefaultLobbyName";
	
	/** The map that will be loaded at the start of gameplay, this needs to be a valid Primary Asset top-level map */
	UPROPERTY(BlueprintReadWrite, Category=Session, meta=(AllowedTypes="Map"))
...
UCLASS(BlueprintType)
class ESOTERICUSER_API UEsotericSession_SearchResult : public UObject
{
	GENERATED_BODY()
public:
	/** Returns an internal description of the session, not meant to be humanly readable */
...
/** Delegates called when a session search completed */
DECLARE_MULTICAST_DELEGATE_TwoParams(FEsotericSession_FindSessionsFinished, bool bSucceeded, const FText& ErrorMessage);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FEsotericSession_FindSessionsFinishedDynamic, bool, bSucceeded, FText, ErrorMessage);

/** Request object describing a session search, this object will be updated once the search has completed */
UCLASS(BlueprintType)
class ESOTERICUSER_API UEsotericSession_SearchSessionRequest : public UObject
{
	GENERATED_BODY()
public:
	/** Indicates if the is looking for full online games or a different type like LAN */
	UPROPERTY(BlueprintReadWrite, Category = Session)
	EEsotericSessionOnlineMode OnlineMode;
	...
	/** String of text contained in the lobbies to search for */
	UPROPERTY(BlueprintReadWrite, Category=Session)
	FString SearchString = "";

	/** The List of all found sessions, will be valid when OnSearchFinished is called */
	UPROPERTY(BlueprintReadOnly, Category=Session)
	TArray<TObjectPtr<UEsotericSession_SearchResult>> Results;

	/** Native Delegate called when a session search completed */
	FEsotericSession_FindSessionsFinished OnSearchFinished;

	/** Called by subsystem to execute finished delegates */
	void NotifySearchFinished(bool bSucceeded, const FText& ErrorMessage) const;

private:
	/** Delegate called when a session search completed */
	UPROPERTY(BlueprintAssignable, Category = "Events", meta = (DisplayName = "On Search Finished", AllowPrivateAccess = true))
	FEsotericSession_FindSessionsFinishedDynamic K2_OnSearchFinished;
};
 ...
 */
DECLARE_MULTICAST_DELEGATE_ThreeParams(FEsotericSessionOnUserRequestedSession, const FPlatformUserId& /*LocalPlatformUserId*/, UEsotericSession_SearchResult* /*RequestedSession*/, const FOnlineResultInformation& /*RequestedSessionResult*/);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FEsotericSessionOnUserRequestedSession_Dynamic, const FPlatformUserId&, LocalPlatformUserId, UEsotericSession_SearchResult*, RequestedSession, const FOnlineResultInformation&, RequestedSessionResult);
 ...
 */
DECLARE_MULTICAST_DELEGATE_OneParam(FEsotericSessionOnJoinSessionComplete, const FOnlineResultInformation& /*Result*/);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FEsotericSessionOnJoinSessionComplete_Dynamic, const FOnlineResultInformation&, Result);
 ...
 * @param Result result of the session join
 */
DECLARE_MULTICAST_DELEGATE_OneParam(FEsotericSessionOnCreateSessionComplete, const FOnlineResultInformation& /*Result*/);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FEsotericSessionOnCreateSessionComplete_Dynamic, const FOnlineResultInformation&, Result);
 ...
 */
DECLARE_MULTICAST_DELEGATE_OneParam(FEsotericSessionOnPreClientTravel, FString& /*URL*/);
 ...
 */
UENUM(BlueprintType)
enum class EEsotericSessionInformationState : uint8
{
	OutOfGame,
	Matchmaking,
	InGame
};
DECLARE_MULTICAST_DELEGATE_ThreeParams(FEsotericSessionOnSessionInformationChanged, EEsotericSessionInformationState /*SessionStatus*/, const FString& /*GameMode*/, const FString& /*MapName*/);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FEsotericSessionOnSessionInformationChanged_Dynamic, EEsotericSessionInformationState, SessionStatus, const FString&, GameMode, const FString&, MapName);
...
class ESOTERICUSER_API UEsotericSessionSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
public:
	...
	UFUNCTION(BlueprintCallable, Category = Session)
	virtual UEsotericSession_HostSessionRequest* CreateHostSessionRequest();
	...
	UFUNCTION(BlueprintCallable, Category = Session)
	virtual UEsotericSession_SearchSessionRequest* CreateSearchSessionRequest();
	
	/** Creates a new online game using the session request information, if successful this will start a hard map transfer */
	UFUNCTION(BlueprintCallable, Category=Session)
	virtual void HostSession(APlayerController* HostingPlayer, UEsotericSession_HostSessionRequest* Request);

	/** Starts a process to look for existing sessions or create a new one if no viable sessions are found */
	UFUNCTION(BlueprintCallable, Category=Session)
	virtual void QuickPlaySession(APlayerController* JoiningOrHostingPlayer, UEsotericSession_HostSessionRequest* HostRequest);
	
	/** Starts process to join an existing session, if successful this will connect to the specified server */
	UFUNCTION(BlueprintCallable, Category=Session)
	virtual void JoinSession(APlayerController* JoiningPlayer, UEsotericSession_SearchResult* Request);

	/** Queries online system for the list of join able sessions matching the search request */
	UFUNCTION(BlueprintCallable, Category=Session)
	virtual void FindSessions(APlayerController* SearchingPlayer, UEsotericSession_SearchSessionRequest* Request);
	...
	FEsotericSessionOnUserRequestedSession OnUserRequestedSessionEvent;
	...
	FEsotericSessionOnUserRequestedSession_Dynamic K2_OnUserRequestedSessionEvent;

	/** Native Delegate when a JoinSession call has completed */
	FEsotericSessionOnJoinSessionComplete OnJoinSessionCompleteEvent;
	...
	FEsotericSessionOnJoinSessionComplete_Dynamic K2_OnJoinSessionCompleteEvent;
	
	/** Native Delegate when a CreateSession call has completed */
	FEsotericSessionOnCreateSessionComplete OnCreateSessionCompleteEvent;
	...
	FEsotericSessionOnCreateSessionComplete_Dynamic K2_OnCreateSessionCompleteEvent;
	
	/** Native Delegate when the presentable session information has changed */
	FEsotericSessionOnSessionInformationChanged OnSessionInformationChangedEvent;
	...
	FEsotericSessionOnSessionInformationChanged_Dynamic K2_OnSessionInformationChangedEvent;

	/** Native Delegate for modifying the connect URL prior to a client travel */
	FEsotericSessionOnPreClientTravel OnPreClientTravelEvent;
	
protected:
	// Functions called during the process of creating or joining a session, these can be overridden for game-specific behavior
	
	/** Called to fill in a session request from quick play host settings, can be overridden for game-specific behavior */
	virtual TSharedRef<FEsotericOnlineSearchSettings> CreateQuickPlaySearchSettings(UEsotericSession_HostSessionRequest* HostRequest, UEsotericSession_SearchSessionRequest* SearchRequest);

	/** Called when a quick play search finishes, can be overridden for game-specific behavior */
	virtual void HandleQuickPlaySearchFinished(bool bSucceeded, const FText& ErrorMessage, TWeakObjectPtr<APlayerController> JoiningOrHostingPlayer, TStrongObjectPtr<UEsotericSession_HostSessionRequest> HostRequest);
...
protected:
	...
	void CreateOnlineSessionInternal(const ULocalPlayer* LocalPlayer, const UEsotericSession_HostSessionRequest* Request);
	void FindSessionsInternal(const APlayerController* SearchingPlayer, const TSharedRef<FEsotericOnlineSearchSettings>& InSearchSettings);
	void JoinSessionInternal(const ULocalPlayer* LocalPlayer, const UEsotericSession_SearchResult* Request) const;
	void InternalTravelToSession(const FName SessionName) const;
	void NotifyUserRequestedSession(const FPlatformUserId& PlatformUserId, UEsotericSession_SearchResult* RequestedSession, const FOnlineResultInformation& RequestedSessionResult) const;
	void NotifyJoinSessionComplete(const FOnlineResultInformation& Result) const;
	void NotifyCreateSessionComplete(const FOnlineResultInformation& Result) const;
	void NotifySessionInformationUpdated(EEsotericSessionInformationState SessionStatusStr, const FString& GameMode = FString(), const FString& MapName = FString()) const;
	...
	void OnRegisterJoiningLocalPlayerComplete(const FUniqueNetId& PlayerId, EOnJoinSessionCompleteResult::Type Result) const;
...
protected:
	...
	TSharedPtr<FEsotericOnlineSearchSettings> SearchSettings;

	/** Settings for the current host request */
	TSharedPtr<FEsotericSession_OnlineSessionSettings> HostSettings;
};
